    ...
    if LS.Toggles[string.lower(msg)] ~= nil then
        if LS.Toggles[string.lower(msg)] then
            ...
            LS.Buttons[string.lower(msg)]:SetNormalTexture(
                "Interface\\Addons\\LegendarySettings\\Vectors\\BigButtonNotHighlighted")
        ...
        if a == "cast" then
            ...
            if b == "custom" then
                ...
                C_Timer.After(2.5, function()
                    LS.Queues[b .. c] = false
                end) -- /run print(LegendarySettings.Queues["targetdisarm"])
            end
...
function LS.ShowExportPopup(serializedProfile)
    StaticPopupDialogs["EXPORT_SETTINGS"] = {
        ...
        OnShow = function(self)
            ...
            if builtin then builtin:Hide() end
            if self.button1 then self.button1:Hide() end
            if self.button2 then self.button2:Hide() end
...
function LS.ExportProfile(profileName)
    if profileName and LegendarySettingsDB[LS.SpecID].Profiles[profileName] then
        local profileData = LegendarySettingsDB[LS.SpecID].Profiles[profileName]
...
function LS.ImportSettings(serializedProfile, profileName)
    ...
    if func then
        ...
        if success and type(importedProfile) == "table" then
            ...
            if LegendarySettingsDB[LS.SpecID].Profiles[profileName] then
            ...
            for k, v in pairs(importedProfile) do
                LegendarySettingsDB[LS.SpecID].Profiles[profileName][k] = v
            end

            -- Set the imported profile as the selected profile
            LegendarySettingsDB[LS.SpecID].SelectedProfile = profileName
...
function LS.AddDropdown(tab, minitab, line, variable, label, default, ...)
    ...
    local currentValue =
        LegendarySettingsDB[LS.SpecID].Profiles[LegendarySettingsDB[LS.SpecID].SelectedProfile][variable] or default
...
function LS.InitToggle(label, variable, default, explanation)
    ...
    if numberOfRegisteredToggles <= 15 then
        ...
        LS.Buttons[string.lower(variable)]:SetNormalTexture(
            "Interface\\Addons\\LegendarySettings\\Vectors\\BigButtonNotHighlighted") -- BigButtonS
        LS.Buttons[string.lower(variable)]:SetHighlightTexture("Interface\\Addons\\LegendarySettings\\Vectors\\White",
            "MOD")
        ...
        if LS.Toggles[string.lower(variable)] then
            LS.Buttons[string.lower(variable)]:SetNormalTexture(
                "Interface\\Addons\\LegendarySettings\\Vectors\\BigButton") -- 2ndtry
            LS.Buttons[string.lower(variable)]:SetNormalFontObject(_G["legendaryFontButtonBlack"])
        else
            LS.Buttons[string.lower(variable)]:SetNormalTexture(
                "Interface\\Addons\\LegendarySettings\\Vectors\\BigButtonNotHighlighted") -- BigButtonS
